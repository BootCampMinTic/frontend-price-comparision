...
export class ClientsComponent implements OnInit {
  ...
  createNatural() {
    this.service.createNatural(this.newNatural).subscribe({
      next: () => { this.newNatural = { name: '', lastName: '', documentNumber: '', electronicInvoiceEmail: '', documentTypeId: 1 }; this.showForm.set(false); this.loadAll(); },
    });
  }

  createLegal() {
    this.service.createLegal(this.newLegal).subscribe({
      next: () => { this.newLegal = { companyName: '', documentNumber: '', documentTypeId: 3 }; this.showForm.set(false); this.loadAll(); },
    });
  }
}
